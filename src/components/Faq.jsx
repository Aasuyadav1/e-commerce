  ...
  return (
    <div className=" mt-[50px]">
      <div className="max-w-[700px] mx-auto">
        <h1 className="text-center font-semibold text-[1.8rem]">
          Frequently asked questions
        </h1>
        <p className="text-center mt-[15px]">
          Have questions? Explore our Frequently Asked Questionsmsection for
          quick answers. If your question isn't answered, feel free to reach out
          to our helpful support team.
        </p>
      </div>
      <div className="mt-[80px] flex flex-col gap-[10px]">
        {faqs.map((cur) => {
          return (
            <div
              className="max-w-[700px] w-full   mx-auto px-[10px] py-[10px] border-[#00000047] border-[1px] border-solid rounded-md"
              key={cur.id}
            >
              <div
                className="flex justify-between w-full items-center cursor-pointer"
                onClick={() => clickFaq(cur.id)}
              >
          ...
          transition-all duration-[0.3s] text-[1.3rem] ease-in-out hover:bg-slate-50 hover:shadow-md ${
            cur.toggle ? "rotate-180" : ""
          }`}
                ></i>
              </div>
