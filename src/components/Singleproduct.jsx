...
function Singleproduct() {
    ...
    const fetchProduct = async (url) => {
        try {
            ...
            let suggest = repData.filter((cur)=>
               ...
               <div className='w-full h-[300px] flex justify-center items-center'>
                ...
                <div className='max-w-[800px] w-full mx-auto mt-[20px] px-[5px] py-[5px]'>
                    {productDatas.map((cur) => (
                        <div key={cur.id} className='flex gap-[50px] max-[700px]:gap-[10px] max-[700px]:flex-wrap'>
                            <div className=' max-[700px]:w-full max-[700px]:flex  max-[700px]:justify-center max-[700px]:items-center '>
                                 <img src={cur.image} alt="cur.title" className='max-w-[200px] aspect-square object-contain'  />
                            </div>
                            <div>
                                <p className='mt-[5px] font-semibold text-[1.6rem] leading-none'>{cur.title}</p>
                                <p className='mt-[10px] text-[0.9rem]'>{cur.description}</p>
                                <div className='flex gap-1 items-center mt-[10px]'>
                                  {[1, 2, 3, 4, 5].map((index) => (
                                    <i
                                         key={index}
                                        className={`ri-star-${index <= cur.rating.rate ? 'fill' : index - 0.5 === cur.rating.rate ? 'half-line' : 'line'} text-[#144452] text-[1.2rem]`}
                                    ></i>
                                  ))}
                                  <p>{cur.rating.rate} rate</p>
                                </div>
                                <p className='mt-[5px] text-[1.1rem] font-semibold'>$ {cur.price}</p>
                                <button
                className={`px-[10px] max-w-[200px] w-full py-[5px] bg-[#217293] rounded shadow-sm text-white mt-[30px]`} onClick={() => buttonHandle(cur.id)}
              >
                {cartItems.find((item) => item.id === cur.id) ? 'Remove from Cart' : 'Add to Cart'}
              </button>
                            </div>
                        </div>
                    ))}
